...
# Leading slash matters: these become root-absolute URLs ("/media/x.jpg"),
# so an avatar/cover <img src> resolves the same on every SPA route instead
# of being read relative to the current page (which 404s → broken image).
# Caddy routes /static/* and /media/* to this backend.
STATIC_URL = "/static/"
